...
import { EDIT_SUGGESTIONS, ENTITIES, type EditSuggestion } from '@/components/data'
...
function EditSuggestionModal({
  ...
  return (
    ...
    >
      ...
      <div
        className="relative bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl w-full max-w-lg"
    ...
    <section className="flex flex-col">
      <div className="mb-3">
        ...
        <p className="text-xs text-slate-400 dark:text-zinc-500 mt-0.5">
          Updating materials at once based on regulations changing.
        </p>
      </div>
      <div className="flex-1 rounded-lg border border-slate-200 dark:border-zinc-700 divide-y divide-slate-100 dark:divide-zinc-800 flex flex-col">
        {visibleSuggestions.map((suggestion, i) => {
          ...
          const roundingClasses = `${isFirst ? 'rounded-t-lg' : ''} ${isLast ? 'rounded-b-lg' : ''}`
          const stateClasses = isApplying
            ? 'cursor-default bg-slate-50 dark:bg-zinc-800'
            : isApplied
            ? 'cursor-pointer bg-white dark:bg-zinc-900 hover:bg-slate-50 dark:hover:bg-zinc-800 active:bg-slate-100 dark:active:bg-zinc-700'
            : isBatch
            ? 'cursor-pointer bg-slate-50 dark:bg-zinc-800 hover:bg-slate-100 dark:hover:bg-zinc-700 active:bg-slate-200 dark:active:bg-zinc-600'
            : 'cursor-pointer bg-white dark:bg-zinc-900 hover:bg-slate-50 dark:hover:bg-zinc-800 active:bg-slate-100 dark:active:bg-zinc-700'
          ...
          return (
            <div
              ...
              className={`flex items-center gap-3 px-4 h-[76px] overflow-hidden transition-colors ${roundingClasses} ${stateClasses}`}
            >
              {/* Logo(s) */}
              ...
              <div className={`flex-1 min-w-0 ${isApplying || isApplied ? 'opacity-40' : ''}`}>
                ...
                <p className="text-[11px] text-slate-500 dark:text-zinc-400 mt-0.5 leading-snug truncate">
                  {suggestion.reason}
                </p>
              </div>
              ...
              {isApplied && (
                <div className="flex items-center gap-1 flex-shrink-0">
                  <svg className="w-3.5 h-3.5 text-emerald-500" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
                    <path d="M3 8l3.5 3.5L13 5" />
                  </svg>
